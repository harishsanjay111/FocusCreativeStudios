"use client";

import { Button, ChevronRight } from "@relume_io/relume-ui";
import type { ButtonProps, ImageProps } from "@relume_io/relume-ui";

type FeatureSection = {
  icon: ImageProps;
  heading: string;
  description: string;
  buttons: ButtonProps[];
};

type Props = {
  featureSections: FeatureSection[];
  image: ImageProps;
};

export type Layout34Props = React.ComponentPropsWithoutRef<"section"> & Props;

export const BambooMats = (props: Layout34Props) => {
  const { featureSections, image } = {
    ...Layout34Defaults,
    ...props,
  } as Props;
  return (
    <section className="px-[5%] py-16  lg:py-18">
      <div className="container">
        <div className="grid grid-cols-1 gap-y-12 md:grid-cols-2 md:items-center md:gap-x-12 lg:gap-x-20">
          <div className="grid grid-cols-1 gap-x-6 gap-y-8 py-2">
          <div className="mb-3 text-[2.5rem] font-bold md:mb-4 md:text-1xl">Bamboo Mats</div>

            {featureSections.map((section, index) => (
              <div key={`${section.heading}-${index}`} className="flex self-start">
                
                <div className="mr-6 flex-none self-start">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
</svg>

                </div>

                <div>
                  
                  <h1 className="mb-3 text-xl font-bold md:mb-4 md:text-1xl">{section.heading}</h1>
                  <p>{section.description}</p>
                
                </div>
              </div>
            ))}
          </div>
          <div>
            <img src={image.src} className="w-full object-cover rounded-[10px]" alt={image.alt} />
          </div>
        </div>
      </div>
    </section>
  );
};

const Layout34Defaults: Layout34Props = {
  featureSections: [

    
    {
      
      icon: { src: "https://relume-assets.s3.amazonaws.com/relume-icon.svg", alt: "Relume logo 1" },
      heading: "Eco-friendly Flooring",
      description:
        "Bamboo mats serve as an environmentally sustainable alternative to traditional flooring materials.",
      buttons: [{ title: "Button", variant: "link", size: "link", iconRight: <ChevronRight /> }],
    },
    {
      icon: { src: "https://relume-assets.s3.amazonaws.com/relume-icon.svg", alt: "Relume logo 2" },
      heading: "Natural Aesthetics",
      description:
        "They add a touch of natural beauty to indoor and outdoor spaces with their unique grain patterns and warm tones.",
      buttons: [{ title: "Button", variant: "link", size: "link", iconRight: <ChevronRight /> }],
    },
    {
      icon: { src: "https://relume-assets.s3.amazonaws.com/relume-icon.svg", alt: "Relume logo 3" },
      heading: "Health Benefits",
      description:
        "Bamboo mats are hypoallergenic and resistant to dust mites, making them a healthier choice for allergy sufferers.",
      buttons: [{ title: "Button", variant: "link", size: "link", iconRight: <ChevronRight /> }],
    },
  ],
  image: {
    src: "./bamboomatscollege.png",
    alt: "Placeholder image",
  },
};
